...
def lambda_handler(event, context):  # pylint: disable=unused-argument
    ...
    # Generate Zero Trust Evidence Report
    from zt_report import generate_zt_report
    from zt_control_mapping import export_mapping_csv, export_mapping_json

    zt_html_key = "shca/analysis_summary_in_html/nist80053_zero_trust_evidence.html"
    zt_csv_key = "shca/zero_trust_mapping/zt_mapping.csv"
    zt_json_key = "shca/zero_trust_mapping/zt_mapping.json"

    summary_for_zt = get_dataframe_from_s3(securityhub_summary_csv_key)
    zt_report_html = generate_zt_report(clean_condensed_data, summary_for_zt)
    write_and_upload_report(zt_report_html, zt_html_key)

    # Export mapping files for customer review
    s3_client.put_object(Body=export_mapping_csv(), Bucket=bucket_name, Key=zt_csv_key)
    s3_client.put_object(Body=export_mapping_json(), Bucket=bucket_name, Key=zt_json_key)
    logger.info("Zero Trust Evidence Report and mapping files written to S3.")
